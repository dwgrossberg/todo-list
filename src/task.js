...
const Task = (project, title, dueDate, priority, details, complete) => {
  ...
  const changeTaskProject = (oldProject, newProject, taskToChange) => {
    ...
    const oldProjectIndex = taskMaster.projectList.findIndex(
      (project) => project.project.name === oldProject
    );
    const taskIndex = Array.from(
      taskMaster.projectList[oldProjectIndex].project.tasks
    ).indexOf(taskToChange);
    const taskToMove =
      taskMaster.projectList[oldProjectIndex].delTask(taskIndex)[0];
    const newProjectIndex = taskMaster.projectList.findIndex(
      (project) => project.project.name === newProject
    );
